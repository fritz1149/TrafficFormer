...
def process_one_label(session_pcap_path,key,payload_length,payload_packet,samples,label_id,start_index=76):
    ...
    print(f'len of target_all_files of {session_pcap_path[key]}: {len(target_all_files)}')
    ...
    if len(target_all_files)>samples:
        random.seed(10)
        r_files = random.sample(target_all_files, samples)
    else:
        r_files = target_all_files
    print(f'len of r_files: {len(r_files)}')
    ...
    for r_f in  r_files:
        try:
            ...
            error_info = traceback.format_exception(exc_type, exc_value, exc_traceback)
...
def generation_multiP(pcap_path, samples, dataset_save_path, payload_length = 64, payload_packet = 5, start_index=76):
    ...
    for parent, dirs, files in os.walk(pcap_path):
        ...
        for dir in label_name_list:
            session_pcap_path[dir] = str(Path(pcap_path, dir))
        break
    ...
    # pool.close()
    # pool.join()
    for key in tqdm(session_pcap_path.keys()):
...
def convert_splitcap(pcapng_path, pcap_path,pcap_split_path,is_pcap_label=False):
    ...
    for p,dd,ff in os.walk(pcap_split_path):
        for dir in dd:
            for _,_,ff in os.walk(pcap_split_path+ "/" + dir):
                for f in ff:
                    file_size = float(size_format(os.path.getsize(pcap_split_path + "/" + dir + "/" + f)))
                    # 2KB
                    if file_size < 2: #remove small pcap
                        os.remove(pcap_split_path + "/" + dir + "/" + f)
                    ...
                    if file_size > 10240: #10MB  split again big pcap
                        ...
                        cmd = "editcap -i 300 {} {}".format(pcap_split_path + "/" + dir + "/" + f, pcap_split_path + "/" + dir + "/" + f)
                        os.system(cmd)
                        os.system("rm {}".format(pcap_split_path + "/" + dir + "/" + f))
                break
        break
    ...
    for p,dd,ff in os.walk(pcap_split_path):
        for dir in dd:
            for _,_,ff in os.walk(pcap_split_path + "/" + dir):
                print(dir,len(ff))
                if len(ff)<10:
                    shutil.rmtree(pcap_split_path + "/" + dir)
                    print("remove class: %s for its flow size is less than 10." % (pcap_split_path + "/" + dir))
                else:
                    all_flows.append(len(ff))
        break
    print("all flows: ",sum(all_flows), len(all_flows))

def dataset_extract(tmp_path, dataset_save_path, features):
    
    print("read dataset from json file.")
    with open(tmp_path + "/dataset.json","r") as f:
        dataset = json.load(f)
    ...
    # save bytes to tsv
    write_dataset_tsv(data_train['datagram'], data_train['label'], dataset_save_path, "train")
    write_dataset_tsv(data_test['datagram'], data_test['label'], dataset_save_path, "test")
    write_dataset_tsv(data_val['datagram'], data_val['label'], dataset_save_path, "valid")
    print("finish generating pre-train's datagram dataset.\nPlease check in %s" % dataset_save_path)
...
def main(pcap_path, pcap_split_path, samples_per_class, class_num, tmp_path, data_save_path):
    # convert_splitcap(None, pcap_path, pcap_split_path, True)
    generation_multiP(pcap_split_path, samples_per_class, tmp_path)
    # dataset_extract(tmp_path, data_save_path, ['datagram', 'length', 'time', 'direction', 'messae_type'])
